...
| 134 MB | 通る |
| 201 MB | **通るときと落ちるときがある** |
| 325 MB | 落ちる |
| 341 MB | 落ちる |

同じサイズなら、ノード 1 個でも Laya の数千ノードでも同じ結果になる。
端末のメモリも無関係で、連続した 1.3 GB の確保も、wasm ヒープを 2.5 GB まで
grow するのも通る。106 バイトのモデルは両バックエンドで動く。

201 MB が「通るときと落ちるときがある」のが効いている。**確率的に落ちる以上、
量子化で小さくしても解決にならない。** int4 にしても 180 MB 前後で、
ちょうどその不安定な範囲に入る。たまにブラウザを巻き込んで落ちる機能は、
無い方がましなので、WebKit には既定で勧めない (`?model=` で試すことはできる)。

測定に使ったモデル (ノード 1 個でサイズだけ違う ONNX と、int8 量子化版) は
役目を終えたので削除した。数字は上の表と `web/src/laya/capability.ts` に残してある。
...
上は 39 件 (bot 31 / human 8) での数字。あとで 46 件 (bot 30 / human 16) を
集めて測り直すと、**Laya 単独は 0.708** まで落ちた。human が増えた分、
誤検知が見えるようになったため。以降の数字はすべて 46 件のもの。
...
測った精度は落ちる (46 件で合成 0.842 に対し Laya 単独 0.708)。
理由を説明できる採点器が 1 つある方が、食い違ったときに突き合わせが要る
2 つより扱いやすい、という判断で交換した。統計は計算を続けていて、
判定の横に事実として並ぶ。

## しきい値

Laya のスコアは **人間にも高い値を付ける**。46 件のラベルで、bot の平均 0.834 に対し
human の平均が 0.751 しかない。分離しているのは上端だけ。

| しきい値 | 再現率 | 誤検知率 | 精度 |
|---|---|---|---|
| 0.63 | 0.77 | **0.62** | 0.70 |
| 0.95 | 0.60 | 0.31 | 0.78 |
| **0.99 (既定)** | **0.50** | **0.12** | **0.88** |
| 0.995 | 0.50 | 0.06 | 0.94 |

既定は **0.99**。**申告された bot の半分しか捕まえられない**が、人間を bot と
呼ぶのが 8 人に 1 人程度に収まる。半分取り逃すのは、人を誤って名指ししない
ための代償。

0.63 は合成スコア用に決めた値で、統計を外した時点で成立しなくなっていた。
それに気づかず既定のままにしていたため、一時期は人間の 6 割を bot と
判定していた。
